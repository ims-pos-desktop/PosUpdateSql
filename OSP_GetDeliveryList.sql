...
SELECT TM.DIVISION, TM.PhiscalID, TM.VCHRNO, TM.TRNAC, TP.MCODE, TP.BC BARCODE, TP.RATE, ISNULL(TPS.CONFACTOR,1) CONFACTOR, TP.ALTRATE, TP.REALRATE, TP.ALTUNIT, TP.UNIT, 
	...
	TM.REMARKS, TM.TOTALFLATDISCOUNT,TP.FLATDISCOUNT,TP.INDDISCOUNT,TM.PRESCRIBEBY INTO #DELIVERIES FROM INVMAIN TM 
...
SELECT A.* INTO #TRNPROD FROM RMD_TRNPROD A JOIN (SELECT DISTINCT VCHRNO FROM #DELIVERIES) B ON A.INVOICENO = B.VCHRNO
...
A.WAREHOUSE,A.REMARKS, A.TOTALFLATDISCOUNT,A.FLATDISCOUNT,A.INDDISCOUNT,A.PRESCRIBEBY FROM #DELIVERIES A 
LEFT JOIN MENUITEM MI ON A.MCODE=MI.MCODE
LEFT JOIN 
(
	SELECT * FROM #TRNPROD WHERE VoucherType = 'DR' AND ISNULL(REALQTY_IN,0)>0
...
LEFT JOIN 
(
	SELECT * FROM #TRNPROD WHERE VOUCHERTYPE IN ('SR', 'CN') AND ISNULL(REALQTY_IN,0)>0
