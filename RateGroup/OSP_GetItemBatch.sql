...
SELECT B.BATCHCREATEDATE,P.MCODE, IIF(@bcStock = 1 AND MI.HASVARIANT = 1, P.BC, @BARCODE) BCODE, TRIM(B.BATCHCODE) BATCHCODE, B.MFGDATE, B.EXPDATE, COALESCE(C.RG_RATE, B.SELLRATEBEFORETAX) SELLRATEBEFORETAX, ISNULL(C.RG_MRP, B.MRP) MRP, LABELMRP, SUM(REALQTY_IN-REALQTY) STOCK,       
