...
(
	...
	CASE WHEN M.VOUCHERTYPE IN ('PI','DN') THEN 'PI' 
		 WHEN M.VoucherType IN ('SI', 'TI','CN') THEN 'TI' ELSE M.VoucherType END VoucherType, ISNULL(ACID,'') ACID,
	...
	LEFT JOIN RMD_ACLIST A ON IIF(M.TRNMODE = 'CREDIT' ,  ISNULL(M.TRNAC ,''), ISNULL(M.PARAC , '')) = A.ACID
	...
	CASE WHEN A.PType = 'V' THEN 'PI' 
		 WHEN A.PType = 'C' THEN 'TI' ELSE '~' END VoucherType, ISNULL(ACID,'') ACID, 
	...
	FROM RMD_TRNMAIN M
	JOIN RMD_ACLIST A ON ISNULL(M.TRNAC , '') = A.ACID
...
GROUP BY A.ACID, ACNAME, VATNO, VOUCHERTYPE, O.BAL, C.BAL
