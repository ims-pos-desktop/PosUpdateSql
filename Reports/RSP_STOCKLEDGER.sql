...
--SELECT @MCODE='M136', @WAREHOUSE='%', @DATE1='2020-07-16', @DATE2='2026-07-16', @BYBARCODE=1, /* @SELECTEDITEMLIST='m100,M175',*/ @FYID = '78/79', @DIVISION = 'HAV', @WAREHOUSE = '%'

DECLARE @RATEMODE TINYINT
SELECT @RATEMODE = enableratediscount, @BYBARCODE = CONVERT(TINYINT, EnableBarcodeDetails) | ISNULL(HASVEHICLESALE,0) FROM SETTING
...
(
	...
	OPBAL as bal,NULL AS REMARKS,NULL AS TERMINAL,null as division, NULL BC,NULL COLOR,NULL SIZE, NULL VEHICLEENGINENO, NULL VEHICLECHASISNO, NULL VEHICLEREGISTRATIONNO,  1 SNO FROM 
	(
	...
	UNION ALL
	SELECT Z.MCODE, TRNDATE, TRNTIME, BSDATE, VCHRNO, CHALANNO, PARTYNAME, VATNO, UNIT, QUANTITY, C.BASEUNIT, RATE, REALQTY_IN, REALQTY_OUT, BAL, REMARKS, TERMINAL, DIVISION
	, BC, COLOR, SIZE, VEHICLEENGINENO, VEHICLECHASISNO, VEHICLEREGISTRATIONNO, 2 SNO	FROM
	(
		...
		null as bal,M.REMARKS,M.TERMINAL,m.division,N.BC,BD.COLOR,BD.SIZE, BD.VEHICLEENGINENO, BD.VEHICLECHASISNO, BD.VEHICLEREGISTRATIONNO, N.QUANTITY FROM 
		(
...
NULL REMARKS,NULL TERMINAL,NULL DIVISION,NULL BC,NULL COLOR,NULL SIZE, NULL VEHICLEENGINENO, NULL VEHICLECHASISNO, NULL VEHICLEREGISTRATIONNO, '' ORD, 2 SNO
FROM #RESULTTABLE A JOIN MENUITEM M ON A.MCODE = M.MCODE
UNION ALL
SELECT MCODE,TRNDATE,TRNTIME,BSDATE,VCHRNO,CHALANNO,PARTYNAME,VATNO,
BUNIT, CONVERT(DECIMAL(18,2), RATE), CONVERT(DECIMAL(18,2), REALQTY_IN) , CONVERT(DECIMAL(18,2), REALQTY) 
, CONVERT(DECIMAL(18,2), SUM(ISNULL(REALQTY_IN,0)-ISNULL(REALQTY,0)) OVER(ORDER BY TRNDATE,CONVERT(DATETIME, TRNTIME), VCHRNO RANGE UNBOUNDED PRECEDING)) BALANCE,
REMARKS,TERMINAL,DIVISION
,IIF(@BYBARCODE=1,BC,'')BARCODE
,IIF(@BYBARCODE=1,COLOR,'')COLOR
,IIF(@BYBARCODE=1,SIZE,'')SIZE
,IIF(@BYBARCODE=1,VEHICLEENGINENO,'') VEHICLEENGINENO
,IIF(@BYBARCODE=1,VEHICLECHASISNO,'') VEHICLECHASISNO
,IIF(@BYBARCODE=1,VEHICLEREGISTRATIONNO,'') VEHICLEREGISTRATIONNO,'A' ORD, SNO
FROM #RESULTTABLE 
UNION ALL
...
NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'B' ORD, 2 SNO
FROM #RESULTTABLE 
GROUP BY MCODE 
UNION ALL
...
NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'C' ORD, 2 SNO
FROM #RESULTTABLE
UNION ALL
SELECT 'Z' MCODE, NULL TRNDATE,NULL TRNTIME,NULL BSDATE,NULL VCHRNO,NULL CHALANNO,'GRAND TOTAL' PARTYNAME,NULL VATNO,
NULL UNIT,NULL RATE,SUM(REALQTY_IN),SUM(REALQTY),SUM(REALQTY_IN)-SUM(REALQTY) BALANCE,
NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'D' ORD, 2 SNO
FROM #RESULTTABLE 
