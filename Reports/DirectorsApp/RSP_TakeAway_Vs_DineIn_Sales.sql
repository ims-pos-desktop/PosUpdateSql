...
as 
	...
	where LEFT(tm.VCHRNO,2) IN ('SI','RE','TI') and IIF(@DAYEND = 0 , tm.TRNDATE , tm.TRN_DATE) between @DATE1 and @DATE2 AND TM.DIVISION LIKE @DIV
	order by IIF(@DAYEND = 0 , tm.TRNDATE , tm.TRN_DATE)
