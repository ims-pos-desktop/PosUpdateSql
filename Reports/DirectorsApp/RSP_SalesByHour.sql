...
SELECT IIF(@DAYEND = 0 , M.TRNDATE , M.TRN_DATE) TRNDATE , M.TRNTIME, T.TRNMODE, (T.AMOUNT - ISNULL(T.CHANGE,0)) * IIF(M.VoucherType IN ('RE','CN'), -1, 1) NETAMNT
...
WHERE M.DIVISION LIKE @DIV AND IIF(@DAYEND = 0 , M.TRNDATE , M.TRN_DATE) BETWEEN @DATE1 AND @DATE2
