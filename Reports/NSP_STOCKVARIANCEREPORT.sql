...
declare @CHK_dayStartDateWise tinyint = 0
select @CHK_dayStartDateWise = isnull(EnableCompulsoryDayEnd,0) from SETTING
...
ELSE
	...
	WHERE AV.VCHRNO IS NULL AND X.MGROUP LIKE @MGROUP 
	AND iif(@CHK_dayStartDateWise = 1 and b.vouchertype in ('SI','TI','CN','RE','RR') , B.trn_date , B.TRNDATE) <=@DATE AND B.PhiscalID = @FYID AND B.DIVISION LIKE @DIVISION
...
ELSE IF @FLAG = 7	-- PHYSICAL STOCK ONLY	
	...
	AND A.BATCH LIKE @BATCH AND A.TRNDATE = @DATE AND WAREHOUSE LIKE @WARE AND C.MGROUP LIKE @MGROUP  AND A.PhiscalID = @FYID AND C.PTYPE < 10
