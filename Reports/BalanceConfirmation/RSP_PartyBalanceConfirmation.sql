...
--SELECT @DATE1 = '16 JUL 2024', @DATE2 = '16 JUL 2025', @PHISCALID = '81/82', @COMPANYID = '%', @ACID = 'PA129'
...
SUM(IIF(VoucherType IN ('PI','CP', 'PR'), TAXABLE, 0) * IIF(VoucherType IN ('PR'), -1,1)) PURCHASE_TAXABLE,
SUM(IIF(VoucherType IN ('SI','TI','RE'), NONTAXABLE, 0) * IIF(VoucherType IN ('RE'), -1,1)) SALES_NONTAXABLE,
SUM(IIF(VoucherType IN ('PI','CP' , 'PR'), NONTAXABLE, 0) * IIF(VoucherType IN ('PR'), -1,1)) PURCHASE_NONTAXABLE,
SUM(IIF(VoucherType IN ('SI','TI','RE'), VATAMNT, 0) * IIF(VoucherType IN ('RE'), -1,1)) SALES_VAT,
SUM(IIF(VoucherType IN ('PI','CP', 'PR'), VATAMNT, 0) * IIF(VoucherType IN ('PR'), -1,1)) PURCHASE_VAT,
...
INTO #RESULT2 FROM RMD_TRNMAIN WHERE LEFT(VCHRNO,2) IN ('SI','TI','RE', 'PI', 'PR', 'CN','DN','CP')
