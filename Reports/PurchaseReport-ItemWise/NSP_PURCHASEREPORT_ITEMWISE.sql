...
--SET @DATE1 = '2026-01-04 00:00:00'; SET @DATE2 = '2026-01-04' 
--SELECT @DIV = 'BNS', @TREE = 1
...
ELSE
	...
	X.MGROUP,X.MCAT,X.BASEUNIT UNIT,
	CASE WHEN @DIVISIONWISEREPORT = 1 THEN Y.NAME ELSE '' END DIVISION_NAME,ISNULL(X.SUPCODE,'')SUPCODE,ISNULL(X.PTYPE,0) PTYPE
	INTO #REPDATA
	FROM RMD_TRNPROD B 
	INNER JOIN RMD_TRNMAIN A ON A.VCHRNO = B.VCHRNO AND A.DIVISION = B.DIVISION AND ISNULL(A.PhiscalID,'') = ISNULL(B.PHISCALID,'')
	...
	AND X.MGROUP LIKE @MGROUP AND ISNULL(X.MCAT,'') LIKE @MCAT
...
ELSE
	...
	BEGIN
		...
		SUM(NONTAXABLE)NONTAXABLE,SUM(VAMNT) VAMNT,SUM(NETAMNT) NETAMNT,X.ACNAME SUPPLIER,DIVISION_NAME,MCODE MC 
		INTO #RESULT 
		FROM #REPDATA A 
		LEFT JOIN RMD_ACLIST X ON A.SUPCODE = X.ACID
		...
		INTO #TREE 
		FROM TreeExpand_function ('MI','PRODUCT LIST',0) AS A 
		LEFT JOIN #RESULT B ON A.MCODE = B.MC 
		...
		WHERE SQTY <> 0 or RQTY <>0 or NETQTY <>0 or TOTVALUE <>0 or DISCOUNT <> 0 or SCHARGE <> 0 or NETSALE <> 0 or TAXABLE <> 0 or NONTAXABLE <> 0 OR VAMNT <> 0 OR NETAMNT <> 0 AND ITEMCODE = '8.458'
		ORDER BY ID 
