...
BEGIN
	...
	AND (WAREHOUSE IN (SELECT NAME FROM RMD_WAREHOUSE WHERE ISNULL(IsAdjustment,0) =0 AND ISNULL(ISVIRTUAL,0)=0 AND (@DIVISION = '%' OR DIVISION = @DIVISION)) OR (WAREHOUSE = 'DefaultDeliveryWarehouse' AND (@DIVISION = '%' OR A.DIVISION = @DIVISION))) AND A.PHISCALID = @FYID
	GOTO GOEND
END
...
SELECT A.WAREHOUSE , A.MCODE , A.STOCKQTY , IIF(A.WAREHOUSE = 'DefaultDeliveryWarehouse' , A.DIVISION , W.DIVISION) DIVISION
INTO #STOCK1 FROM
(
	SELECT  A.WAREHOUSE, A.MCODE,SUM(REALQTY_IN)-SUM(REALQTY) AS STOCKQTY , A.DIVISION FROM RMD_TRNPROD A
	INNER JOIN RMD_TRNMAIN B ON A.VCHRNO= B.VCHRNO
	WHERE A.PhiscalID = @FYID AND (B.TRNDATE <=@DATE OR A.VoucherType = 'OP')
	GROUP BY A.MCODE, A.WAREHOUSE , A.DIVISION  HAVING SUM(REALQTY_IN)-SUM(REALQTY) <> 0
) A
...
(
	...
	AND (ISNULL(A.WAREHOUSE,'') IN (SELECT NAME FROM RMD_WAREHOUSE WHERE ISNULL(IsAdjustment,0) =0 AND ISNULL(ISVIRTUAL,0)=0 AND (@SampleAllData = 1 OR (@DIVISION = '%' OR A.DIVISION = @DIVISION))) OR (ISNULL(A.WAREHOUSE,'') = 'DefaultDeliveryWarehouse' AND (@DIVISION = '%' OR A.DIVISION = @DIVISION)))
