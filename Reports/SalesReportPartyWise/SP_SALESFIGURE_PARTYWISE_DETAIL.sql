...
IF @PTYPE= 0 	-- CUSTOMER WISE
	SELECT ca.AREA_NAME,ca.CATEGORY_NAME, COALESCE(CP.CUSTNAME, CA.ACNAME, A.BillTo) ACNAME, COALESCE(CA.VATNO, CP.PANNO, A.BillToTel) VATNO, A.MENUCODE, A.DESCA, A.QUANTITY, A.RATE, A.AMOUNT, A.[LOYALTY DISCOUNT(%)], A.DISCOUNT
	...
	WHERE (@AREA_ID = 0 OR CA.AREA_ID = @AREA_ID)
			...
			AND (@ACID = '%' OR COALESCE(CA.ACID,@ACID) = @ACID)
